            ...
            if file == 'page.tsx':
                ...
                if '[' in route and ']' in route:
                    ...
                    if 'export const metadata' in content:
                        ...
                        pattern = r'(export default function|export default async function|const [A-Za-z0-9]+ = \(\) =>|const [A-Za-z0-9]+ = function)'
                        # just find export default
                        pattern = r'(export default\s+(?:async\s+)?function|const\s+[A-Za-z0-9]+\s*=\s*(?:async\s+)?\([^)]*\)\s*=>)'
                        
                        # simpler approach: just find 'export default'
                        # but some might use export default function Component
