...
// Update page.tsx schemas with matching faqs
const pagePath = path.join(targetDir, 'page.tsx');
let pageCode = fs.readFileSync(pagePath, 'utf8');

// Replace FAQ array in page.tsx
const newFaqJson = faqs.map(f => `          {
            "@type": "Question",
            "name": "${f.q.replace(/"/g, '\\"')}",
            "acceptedAnswer": {
              "@type": "Answer",
...
pageCode = pageCode.replace(/"mainEntity":\s*\[[\s\S]*?\]\s*\},/m, `"mainEntity": [\n${newFaqJson}\n        ]\n      },`);
fs.writeFileSync(pagePath, pageCode, 'utf8');
console.log('page.tsx synced');
...
console.log('BusinessLoanSettlementProcessClient.tsx updated with calibrated paragraph style');
