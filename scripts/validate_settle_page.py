...
def check_file_requirements(file_paths):
    ...
    # Removing tags and common boilerplate to get a more accurate count
    ...
    for brand in brands:
        ...
        if idx == -1:
            ...
            alt_brand = brand.replace("Solutions", " Solutions").replace("Loans", " Loans")
            idx = combined_content.find(alt_brand)
    ...
    if review_count < 5:
        # Fallback to JSX review blocks pattern (looking for "author")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python validate_settle_page.py <file1> <file2> ...")
        sys.exit(1)
